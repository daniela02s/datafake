...
if area == "Bancário (Crédito)":
    ...
    # SALVAMENTO AUTOMÁTICO: Garante que os arquivos no seu disco fiquem sincronizados para análise
    df_c.to_csv('clientes.csv', index=False)
    df_t.to_csv('transacoes.csv', index=False)
    df_s.to_csv('status_credito.csv', index=False)
    st.success("✅ Arquivos sincronizados localmente para análise!")
